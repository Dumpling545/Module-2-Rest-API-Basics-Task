...
public class TagValidatorImpl implements TagValidator {
	...
	@Override
	public void validateTag(TagDTO tag) throws ServiceException {
		validateTagName(tag.getName());
	}

	@Override
	public void validateTagName(String tagName) throws ServiceException {
		if (MIN_NAME_LENGTH > 0 && (tagName == null || tagName.isBlank())) {
			throw new InvalidTagNameException(0, MIN_NAME_LENGTH, MAX_NAME_LENGTH);
		}
		if (tagName.length() < MIN_NAME_LENGTH || tagName.length() > MAX_NAME_LENGTH) {
			throw new InvalidTagNameException(tagName.length(), MIN_NAME_LENGTH, MAX_NAME_LENGTH);
		}
	}
}
